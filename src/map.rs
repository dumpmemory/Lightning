    ...
    > Table<K, V, A, ALLOC, H>
{
    ...
    pub fn get(&self, key: &K, fkey: usize, read_attachment: bool) -> Option<(usize, Option<V>)> {
        ...
        loop {
            ...
            if let Some(new_chunk) = new_chunk {
                if let Some((val, _idx, _addr, aitem)) =
                    self.get_from_chunk(&*new_chunk, hash, key, fkey, &guard)
                {
                    match val.parsed {
    ...
    pub fn insert(
        ...
        loop {
            ...
            let mod_op = match op {
                InsertOp::Insert => ModOp::Insert(attachment_fval),
                InsertOp::UpsertFast => ModOp::UpsertFastVal(fvalue),
                InsertOp::TryInsert => ModOp::AttemptInsert(attachment_fval),
            };
            let value_insertion =
            ...
            if let Some(new_chunk) = new_chunk {
                    ...
                    self.get_from_chunk(chunk, hash, &key, fkey, &guard)
                {
                    ...
                    if old_fval != SENTINEL_VALUE
                        ...
                        && old_fval != LOCKED_VALUE
                    {
                        let act_old_val = if Self::CAN_ATTACH {
                            A::decode_fval(old_fval)
                        } else {
                            old_fval
                        };
                        if let Some(new_val) = func(act_old_val) {
                            let fval = if Self::CAN_ATTACH {
                                attachment.with_tag(new_val)
                            } else {
                                new_val
                            };
    ...
    fn get_from_chunk<'a>(
        ...
        while counter < cap {
            ...
            let attachment = chunk.attachment.load(v.raw);
            if k == fkey && attachment.probe(key) {
                match v.parsed {
                    ParsedValue::Empty => {}
                    _ => return Some((v, idx, addr, attachment)),
                }
            } else if k == EMPTY_KEY {
                return None;
            }
    ...
    fn modify_entry<'a>(
        ...
        while count <= cap {
            ...
            if k == fkey && attachment.probe(&key) {
                            ...
                            &ModOp::SwapFastVal(ref swap) => {
                                ...
                                match &val.parsed {
                                    ParsedValue::Val(pval) => {
                                        ...
                                        if Self::CAN_ATTACH {
                                            pval = A::decode_fval(pval);
                                        }
                                        if let Some(v) = swap(pval) {
                                            let attach_v = if Self::CAN_ATTACH {
                                                attachment.with_tag(v)
                                            } else {
                                                v
                                            };
                                            if self.cas_value(addr, val.raw, attach_v).is_ok() {
                                                return ModResult::Replaced(val.raw, None, idx);
                                            } else {
                                                return ModResult::Fail;
                                            }
                                        } else {
                ...
                match op {
                    ModOp::Insert(fval) | ModOp::AttemptInsert(fval) => {
                        trace!(
        ...
        match op {
            ModOp::Insert(_) | ModOp::AttemptInsert(_) => ModResult::TableFull,
            ModOp::UpsertFastVal(_) => ModResult::TableFull,
    ...
    fn all_from_chunk(&self, chunk: &Chunk<K, V, A, ALLOC>) -> Vec<(usize, usize, K, V)> {
        ...
        while counter < cap {
            ...
            if k != EMPTY_KEY {
                ...
                let attachment = chunk.attachment.load(val_res.raw);
    ...
    fn migrate_entry(
        ...
        let orig = curr_orig;
        let old_attachment = old_chunk_ins.attachment.load(orig);
        ...
        while count < cap {
            ...
            let k = self.get_fast_key(addr);
            let v = self.get_fast_value(addr);
            let new_attachment = new_chunk_ins.attachment.load(v.raw);
            if k == fkey && new_attachment.probe(&key) {
                ...
                if curr_orig == orig {
                    match self.cas_value(old_address, orig, LOCKED_VALUE) {
                        Ok(n) => {
...
impl<K, V, A: Attachment<K, V>, ALLOC: GlobalAlloc + Default> Chunk<K, V, A, ALLOC> {
    fn alloc_chunk(capacity: usize) -> *mut Self {
        ...
        let total_size = self_size_aligned + chunk_size;
...
pub trait Attachment<K, V> {
    ...
    fn new() -> Self;
    fn load<'a>(&self, fvalue: usize) -> Self::Item {
        Self::Item::from_fval(fvalue)
    }

    fn new_fval(key: K, value: V, tag: usize) -> usize {
        let item = Self::Item::new(key, value, tag);
        item.to_fval()
    }

    fn decode_fval(raw: usize) -> usize {
        let item = Self::Item::from_fval(raw);
        item.tag()
    }
}

trait AttachmentItem<K, V>: Sized {
    ...
    fn to_fval(&self) -> usize;
    fn get_key(&self) -> K;
    fn get_value(&self) -> V;
    fn probe(&self, probe_key: &K) -> bool;
    fn tag(&self) -> usize;
    fn with_tag(&self, tag: usize) -> usize;
    unsafe fn erase(&self, guard: &Guard);
}

struct WordAttachment;
#[derive(Copy, Clone)]
struct WordAttachmentItem;
...
impl Attachment<(), ()> for WordAttachment {
    ...
    #[inline(always)]
    fn load<'a>(&self, value: usize) -> Self::Item {
        Self::Item::from_fval(value)
    }
}
...
impl AttachmentItem<(), ()> for WordAttachmentItem {
    ...
    fn new<'a>(key: (), _value: (), _tag: usize) -> Self {
        Self
    }

    fn from_fval<'a>(_fvalue: usize) -> Self {
        Self
    }
    ...
    fn fval_with_new_value<'a>(&self, _value: (), _tag: usize) -> usize {
        0
    }
...
pub struct WordObjectAttachment<T> {
...
impl<T: Clone> AttachmentItem<(), T> for WordObjectAttachmentItem<T> {
    #[inline(always)]
    fn get_value(&self) -> T {
        debug_assert!(self.ptr > NUM_FIX);
        let ptr = unsafe { self.ptr() };
        let bucket = unsafe { ptr.deref() };
        bucket.val.clone()
    }
    ...
    fn new<'a>(_key: (), value: T, tag: usize) -> Self {
        let bucket = WordObjectAttachmentBucket { val: value };
    ...
    fn fval_with_new_value<'a>(&self, value: T, tag: usize) -> usize {
        Self::new((), value, tag).ptr
    }
...
impl<K: Clone + Hash + Eq, V: Clone> HashKVAttachmentItem<K, V> {
    ...
    unsafe fn shared(&self) -> Shared<HashKVAttachmentBucket<K, V>> {
        Shared::<HashKVAttachmentBucket<K, V>>::from_usize(self.ptr)
    }
}
...
impl<K: Clone + Hash + Eq, V: Clone> AttachmentItem<K, V> for HashKVAttachmentItem<K, V> {
    ...
    fn new<'a>(key: K, value: V, tag: usize) -> Self {
        let bucket = HashKVAttachmentBucket { key, value };
        let ptr = Owned::new(bucket).with_tag(tag);
        Self {
            ptr: ptr.into_usize(),
            _marker: PhantomData,
    ...
    unsafe fn erase(&self, guard: &Guard) {
        guard.defer_destroy(Shared::<HashKVAttachmentBucket<K, V>>::from_usize(self.ptr));
    }
...
const PLACEHOLDER_VAL: usize = NUM_FIX + 2;
...
pub type ObjectTable<V, ALLOC, H> = Table<(), V, WordObjectAttachment<V>, ALLOC, H>;
...
const WORD_MUTEX_DATA_BIT_MASK: usize = !0 << 1 >> 1;
    ...
    HashMapReadGuard<'a, K, V, ALLOC, H>
{
    fn new(table: &'a HashTable<K, V, ALLOC>, key: &K) -> Option<Self> {
        ...
        loop {
            ...
            match swap_res {
                SwapResult::Succeed(fval, _idx, chunk) => {
                    let chunk_ref = unsafe { chunk.deref() };
                    let attachment = chunk_ref.attachment.load(fval);
