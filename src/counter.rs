...
use std::{sync::atomic::AtomicUsize, cell::Cell};
...
impl Counter {
    ...
    fn tl_get(&self) -> &AtomicIsize {
        unsafe {
            ...
            if cell.get().is_null() {
                cell.set(Box::into_raw(Box::new(AtomicIsize::new(0))));
            }
            &*(cell.get() as *mut _)
        }
    }
    ...
    pub fn count(&self) -> isize {
        self.counters.iter().filter_map(|c| {
            unsafe {
                c.get().as_ref().map(|c| c.load(Relaxed))
            }
        })
        .sum()
    }
