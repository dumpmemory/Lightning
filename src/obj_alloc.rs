...
pub struct Allocator<T, const B: usize> {
    ...
    thread: ThreadLocal<UnsafeCell<TLAlloc<T, B>>>,
}
...
impl <T, const B: usize> Allocator<T, B> {
    ...
    #[inline(always)]
    fn tl_alloc(&self) -> &UnsafeCell<TLAlloc<T, B>> {
        self.thread.get_or(|| UnsafeCell::new(TLAlloc::new(0, 0, self.shared.clone())))
    }
}
        ...
        if !head.is_null() {
            unsafe {
                ...
                while !next_ptr.is_null() {
                    ...
                    next_ptr = next.next.load(Relaxed, &guard);
                }
            }
        }
    }
    ...
    pub fn push(&mut self, val: T) -> Option<Owned<RingBufferNode<T, B>>> {
        unsafe {
            ...
            if let Err(val) = (&*self.head).buffer.push_front_unsafe(val) {
                ...
                self.head = Box::into_raw(new_buffer);
                self.num_buffer += 1;
            }
            return res;
        }
