...
fn run_and_record_contention<'a, 'b, T: Collection>(
    ...
    let insert_measurement =
        run_and_measure_mix::<T>(Mix::insert_heavy(), 0.25, load, cont, stride);
    write_measurements(
    ...
    println!("Read heavy");
    let read_measurement = run_and_measure_mix::<T>(Mix::read_heavy(), 0.25, load, cont, stride);
    write_measurements(&format!("{}_{}_read.csv", task, name), &read_measurement);

    println!("Uniform");
    let uniform_measurement = run_and_measure_mix::<T>(Mix::uniform(), 0.25, load, cont, stride);
    write_measurements(
